...
■ 1. 에이전트 대화창 (Left Panel)
- 안티그래비티 에이전트와 대화하는 메인 채팅 영역입니다. 질문을 던지거나 오류 메시지를 붙여넣어 작업을 지시할 수 있습니다. 


■ 2. 파일 탐색기 & 작업 공간 (Center Panel)
- 현재 지정된 워크스페이스 폴더 내의 파일 트리와 소스 코드를 편집할 수 있는 코드 에디터 화면입니다. 


■ 3. 로컬 터미널 (Bottom Panel)
- 명령어를 직접 치거나, 에이전트가 제안한 명령어(빌드, 테스트 등)가 실행되는 내부 샌드박스 터미널입니다. 


■ 4. 아티팩트 및 뷰어 탭 (Right Panel)
- 에이전트가 실시간으로 생성한 작업 계획서(implementation_plan.md), 할 일 목록(task.md) 및 분석 문서 등의 아티팩트 결과물들을 모아서 슬라이드나 문서 형태로 예쁘게 렌더링해 주는 특수 공간입니다. 
...
3. 에이전트가 최초 구동 시 해당 폴더의 읽기/쓰기 권한을 요청하는 확인 알림창을 띄우면 반드시 'Allow (허용)'를 눌러주어야 정상 작동합니다. (권한을 막아버리면 에이전트가 코드를 쓰지 못해 바보가 되니 주의하세요,,) 
