...
하단에 깔려 있는 샌드박스 터미널 창의 영역을 마우스로 드래그하여 우측 화면으로 옮기면, 기존 제미나이나 코덱스 채팅 패널처럼 세로형 메인 AI 비서 느낌으로 넓고 편안하게 클로드 코드를 사용하실 수 있습니다. 
1. 터미널(Terminal) 탭 타이틀 영역을 마우스 왼쪽 버튼으로 클릭한 상태를 유지합니다. 
2. 클릭한 채로 마우스를 화면 오른쪽 가장자리(아티팩트 뷰어 영역 근처)로 드래그합니다. 
3. 드래그 중 화면 우측 영역에 반투명한 파란색 가이드 영역이 활성화되면 마우스 버튼을 놓습니다. 
4. 터미널이 우측 세로 패널로 고정되면서, 왼쪽에는 에디터 코드를 띄우고 오른쪽에는 클로드 코드 터미널창을 띄워 대화하는 제미나이 친화적 레이아웃이 완성됩니다. (한 번 설정해 두면 화면을 엄청나게 실속 있게 쓸 수 있습니다,,)
