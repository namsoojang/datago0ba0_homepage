...
> **드라이브는 파일을 전달하는 곳이고, GitHub는 코드와 변경 기록을 보관하는 곳입니다. 실제 웹사이트는 Cloudflare Pages 같은 배포 서비스가 방문자에게 보여 줍니다.**
...
또 하나의 차이는 **되돌리기의 정확도**입니다. 작업 내용을 목적별로 나누어 커밋해 두면, GitHub에서는 "차트 색상을 바꾼 변경"처럼 필요한 기록을 찾아 되돌리기 쉽습니다.
...
구글드라이브나 네이버클라우드는 동기화 설정에 따라 저장한 파일이 자동으로 올라갑니다. Git은 다릅니다. **커밋한 뒤 푸시해야** GitHub에 반영됩니다.

이 과정 덕분에 변경 파일을 먼저 확인하고, 공유해도 되는 내용만 골라 기록할 수 있습니다. 다만 커밋했다고 GitHub에 올라가는 것은 아닙니다. **커밋은 내 PC에 기록하는 일, 푸시는 그 기록을 GitHub로 보내는 일**입니다.
...
5. 로그인 후 설정에서 **2단계 인증(2FA)** 을 켜고 복구 코드를 안전한 곳에 보관하는 것을 권장합니다.
...
2단계 인증은 계정 탈취를 막는 중요한 안전장치입니다. GitHub는 명령줄에서 계정 비밀번호를 받지 않으므로, 처음 푸시할 때 브라우저 로그인이나 Git Credential Manager, 개인용 액세스 토큰(PAT)으로 인증합니다. 2단계 인증을 켰다면 휴대전화를 잃어버릴 때를 대비해 복구 코드도 따로 보관하세요.
...
4. **Add README**, **Add .gitignore**, **Choose a license**는 모두 선택하지 않습니다. 이미 내 PC에 있는 프로젝트를 처음 연결할 때는 빈 저장소로 만들어야 기록이 충돌하지 않습니다.
5. **Create repository**를 누른 뒤 화면에 표시된 HTTPS 주소를 복사합니다.
6. 이 주소는 잠시 뒤 내 PC 폴더와 GitHub 저장소를 연결할 때 사용합니다.

![GitHub 새 저장소 만들기 화면](../assets/images/guide/github/gh-new-repo.png)

`Start with a template`과 `Add license`는 지금 단계에서 건드리지 않아도 됩니다.

`.gitignore`는 아직 Git이 추적하지 않는 파일을 커밋 대상에서 제외합니다. 원본 데이터 폴더, 개인 설정 파일, 키가 담긴 파일을 미리 적어두면 실수를 줄일 수 있습니다. 이미 커밋한 파일은 나중에 `.gitignore`에 적어도 과거 기록에서 사라지지 않습니다. 비밀정보를 올렸다면 먼저 키를 폐기하고 기록 정리를 별도로 해야 합니다.
...
**GitHub는 코드를 두는 곳이지 대용량 데이터를 두는 곳이 아닙니다.** 원본 데이터가 수백 MB라면 저장소에는 집계·요약본만 넣고, 원본은 `.gitignore`로 제외하세요. 대시보드에는 원본 전체보다 집계된 결과가 필요한 경우가 많습니다.
...
> **처음 한 번만 하는 흐름**
>
> `프로젝트 폴더 확인 → 개인정보·원본 데이터 제외 → Git 시작 → 변경 파일 확인 → 첫 커밋 → GitHub 주소 연결 → 푸시`

각 단계는 서로 다른 안전 확인입니다. 특히 푸시하기 전에는 비밀번호, 고객 명단, 원본 데이터처럼 공개하면 안 되는 파일이 섞이지 않았는지 먼저 확인하세요.
...
> **필요하면 풀 → 수정 → 변경 파일 확인 → 커밋 → 푸시 → GitHub에서 확인**
...
- [ ] GitHub 계정을 만들고 2단계 인증 및 복구 코드를 준비했다
