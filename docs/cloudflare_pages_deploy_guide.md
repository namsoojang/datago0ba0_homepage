...
정적 사이트는 별도 서버를 관리할 일이 적고, 무료 범위가 넓으며, 빠르게 배포하기 좋습니다. HTML·CSS·JS와 공개 가능한 데이터 파일로 만든 강의 프로젝트라면 먼저 고려할 만합니다.
...
### 3.1 Pages 만들기 화면으로 이동

왼쪽 메뉴에서 **Workers & Pages**로 이동한 뒤 다음 순서로 선택합니다.

> **Create application → Pages → Connect to Git**

![Cloudflare Workers & Pages 화면의 Create application 버튼](../assets/images/guide/github/cf-workers-pages-create.png)

Cloudflare 화면 문구는 업데이트에 따라 조금 달라질 수 있습니다. 중요한 것은 **Worker 만들기**가 아니라 **Pages에서 Git 저장소 연결하기**를 선택하는 것입니다. `Connect to Git`이 바로 보이지 않으면 화면에서 `Pages` 또는 `Git`이라는 단어를 찾으세요.

### 3.2 GitHub 저장소 연결

**Connect to Git**을 누르고 GitHub를 선택합니다. 처음 연결할 때는 Cloudflare가 저장소를 읽고 새 커밋을 확인할 수 있도록 GitHub 권한을 요청합니다.

직접 파일을 끌어다 놓는 **Direct Upload** 방식도 있지만, 이 방식으로 만든 프로젝트는 나중에 Git 자동 배포 방식으로 바꿀 수 없습니다. 이 가이드에서는 수정할 때마다 자동으로 다시 배포되도록 Git 연결 방식을 사용합니다.

### 3.3 저장소 접근 권한 정하기 (중요)
...
승인이 끝나면 저장소 선택, 배포 설정, 사이트 배포 순서로 진행됩니다.
...
저장소를 고르면 **Set up builds and deployments** 화면이 나옵니다. 정적 대시보드는 아래 항목만 확인하면 됩니다.
...
| Build output directory | 방문자에게 보여 줄 파일이 있는 폴더 | `index.html`이 저장소 최상위에 있다면 **저장소 최상위(`/`)**를 뜻하도록 둡니다. 화면에 따라 빈칸 또는 `/`로 표시될 수 있음 |
...
세 칸(Framework preset, Build command, Build output directory)을 확인하세요. 순수 HTML 프로젝트는 별도 빌드 과정이 없으므로 Build command가 비어 있어도 정상입니다. 다만 `index.html`이 하위 폴더에 있다면 Build output directory에는 그 폴더를 지정해야 합니다.

**Save and Deploy**를 선택하면 배포가 시작됩니다. 진행 로그가 끝나고 성공 상태와 `pages.dev` 주소가 표시되면 완료된 것입니다. 걸리는 시간은 프로젝트와 대기 상황에 따라 달라집니다.
...
즉 필터·정렬·차트처럼 **브라우저 안에서 처리하는 대시보드 기능은 대체로 정적 사이트로 구현할 수 있습니다.** 다만 데이터 크기와 기기 성능에 따라 속도가 달라지므로, 행 수만으로 성능을 단정하지 말고 실제 휴대전화와 PC에서 확인하세요.
...
- **CORS(교차 출처 리소스 공유)**: 내 사이트가 다른 주소의 데이터를 요청할 때, 그 데이터 서버가 허용하지 않으면 브라우저가 안전을 위해 차단합니다. 개발자 도구의 Console에 CORS 오류가 보이면 데이터 제공처의 허용 설정이나 같은 저장소에 파일을 넣는 방법을 확인합니다.
...
특정 인원만 접근하게 하려면 Cloudflare Access로 로그인 관문을 만들 수 있습니다. 다만 Access 정책과 사용자 도메인 설정이 필요한 별도 작업이므로, 사내 자료라면 공개 배포 전에 담당자와 설정을 먼저 확인하세요. **비공개 GitHub 저장소를 연결했다고 배포된 사이트까지 자동으로 비공개가 되는 것은 아닙니다.**
...
| 차트만 비어 있음 | 데이터 파일 경로 또는 CORS | `F12` 또는 `Ctrl+Shift+I`로 개발자 도구를 열고 Console에서 오류 확인. 화면을 공유할 때는 토큰·비밀번호를 가림 |

콘솔의 오류 메시지에서 토큰과 개인정보를 가린 뒤 AI 도구에 붙여넣고 "이 오류의 원인과 확인할 파일을 알려줘"라고 물어보세요. 답을 바로 실행하기보다 수정 대상과 이유가 맞는지 먼저 확인합니다.
