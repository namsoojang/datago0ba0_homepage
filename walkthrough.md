...
9. **구글 애널리틱스(GA4) 연동 및 도메인 대응 (v1.14.0)**:
   - Cafe24에서 새로 구매한 도메인 `datagongbang.kr`에 대응하여 공식 이메일 주소를 `contact@datagongbang.kr`로 변경하였으며, 푸터 저작권 표기 사명을 `DATAGOONGBANG`으로 업데이트했습니다.
   - 사이트 유입 분석 및 트래픽 측정을 위해 구글 애널리틱스 4(GA4)의 `gtag.js` 트래킹 스크립트를 `<head>` 영역에 연동 완료했습니다. (현재는 플레이스홀더 `G-XXXXXXXXXX`가 주입되어 있으며, 발급받은 실제 측정 ID로 교체하여 바로 적용할 수 있습니다.)
   - GA4 스펙 및 외부 리소스 연동 현황을 문서화한 `api.md` 파일을 생성하여 관리 편의성을 극대화했습니다.
