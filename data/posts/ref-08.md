...
> 💡 <strong>안내</strong>: 메세지배달부 Lite는 현재 Google Play Store에서 설치할 수 있습니다. 상세 설치 방법과 슬랙 웹후크 연동 가이드는 <strong>[메세지배달부 사용 가이드 페이지](message-delivery.html)</strong>에서 확인하실 수 있습니다.
...
* <strong>Version 1.0.1</strong>: 메인 UI 중단하기 제어 불일치 결함 패치, 배터리 소모 모니터링 최적화, GitHub CI/CD 빌드 명명 체계 자동화 완성
* <strong>Version 1.0.2</strong>: 보안 위협 오진단 및 브라우저 다운로드 제한 우회를 위한 ZIP 포맷 제공, 홈페이지 문의/개선요청 채널 연동 완성
* <strong>Version 1.0.4 (Google Play 공개 버전)</strong>: Google Play 정책에 맞춰 Lite 버전을 알림 접근 기반으로 정비하고, Play Store 직접 링크 설치 흐름으로 전환
...
데이터공방은 메세지배달부 앱의 전송 품질과 서비스 안정성을 높이기 위해 향후 지속적인 디바이스 필드 테스트 피드백을 반영해 나갈 예정입니다. Play Store 설치 방법과 배터리 제한 해제 팁, 그리고 슬랙 연동 가이드가 필요하신 실무자분들께서는 데이터공방의 <strong>[메세지배달부 사용 가이드 페이지](message-delivery.html)</strong>에서 상세 매뉴얼을 확인하실 수 있습니다.
